...
class EntityPreprocessing:
    ...
    def preprocess(self, articles_dict):
        ...
        spell_checker = self._create_spell_checker_dict(ents_proc,
                                                        self.n_spell_check_thres)
        ents_corrected = [self._spell_check(ent_string, spell_checker)
                          for ent_string in ents_proc]
        # Create preprocessing history for entity strings for reference
        ...
        # Replace entities per article with corrected entity strings
        ...
        ents_final = self._replace_ents_w_final(ents_list_proc,
                                                ents_vocab_count,
                                                articles_dict)
        # Free up memory
        ...
        word_count_mat, voc2id, pmid2id, article_counts = \
            self._compute_word_count_mat(ents_final, ents_vocab_count)
    ...
    @staticmethod
    def _replace_ents_w_corrected(ent_list, ent_vocab_dict):
        replaced_ent_list = []
        for article_ents in ent_list:
            if article_ents is not None:
                new_article_ents = [ent_vocab_dict[ent]['corrected']
                                    for ent in article_ents]
            else:
                new_article_ents = []
            replaced_ent_list.append(new_article_ents)
    ...
    @staticmethod
    def _replace_ents_w_final(ent_list, ent_vocab_dict, articles_dict):
